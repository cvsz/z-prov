...
dispatcher. Z-Prov is a network gateway with three compatibility
...
| Area | zcoder v1.36.0 | Z-Prov v0.3.0 | Upgrade decision |
|---|---|---|---|
...
| API protocol | Mostly Anthropic-specific | Anthropic Messages, OpenAI Chat and Responses | Preserve Z-Prov contract |
| Provider abstraction | Anthropic modules and raw HTTP | Provider client plus route targets | Expand Z-Prov abstraction |
...
| Updates | Source/venv oriented Make targets | HTTPS manifest + SHA-256 + atomic switch | Keep Z-Prov updater |
...
even under a documented audit process. Z-Prov must not copy these constants.
Use:
...
Z-Prov must not add in-process plugin installation. A future plugin
manager must:
...
Never move this into the API gateway. A Z-Prov execution service must use an
...
Z-Prov should count only downstream transient failures and rate-limit failures
...
For Z-Prov, user-controlled URL fetching belongs in an isolated fetch service
...
Z-Prov releases should generate one machine-readable version, SBOM, provenance,
...
gateway or control plane. Z-Prov should remain stateless in the request path.
...
handling. Z-Prov should retain a stable error envelope and map it independently
...
| Capability | Priority | Z-Prov implementation |
|---|---:|---|
...
These may consume Z-Prov's Anthropic/OpenAI-compatible API, but must not execute
...
## Proposed Z-Prov v0.4 work packages
...
Introduce stable aliases such as `z-prov-claude`, but resolve them against the
...
6. Separate `z-prov-agent` design only after the gateway release gates pass.
